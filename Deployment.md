We use [Buddy](https://buddy.works) to handle deploying sites from local to staging/production, when hosting the sites ourselves (sites hosted elsewhere may just be deployed using FTP).
...
**Note: The ssh_password variable needs to be encrypted, to do this you will need to go to your project on Buddy, then go to:**\
**Project Settings -> YAML tools dropdown -> Encrypt sensitive value for YAML**\
**Enter the cPanel account password in here, then use the encrypted value provided for the YAML file.**
